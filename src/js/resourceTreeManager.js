...
export class ResourceTreeManager {
    ...
    getConfig(readOnly=false) {
        return this.initializeJsTree(readOnly);
    }

    initializeJsTree(readOnly) {
        this.currentPath = '/';
        const me = this;
    ...
    setFilter(filter) {
        ...
        if (this.filter) {
            ...
            if (resType) {
                this.leafFilter = (r) => resType.pattern.test(r.extension);
            }
    ...
    resourceClick(resource, ev, args) {
        ...
        this.selectedLeaf = resource;
    }
    resourceDblClick(resource, ev, args) {
        this.selectedLeaf = resource;
        $(this).trigger('tpy:resource-dbl-click', [this.selectedLeaf]);
    }
    ...
    jtNodeSelected(ev, data) {
        ...
        this.getResources(path).then(resources => {
            $.observable(this.files).refresh(
                resources.filter(res => res.type != 'D' && this.leafFilter(res))
